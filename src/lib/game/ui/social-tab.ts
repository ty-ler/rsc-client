...
function drawUiTabSocial(noMenus) {
	...
	if (this.options.mobile) {
		...
		this.surface._drawSprite_from3(this.gameWidth - MENU_WIDTH - 3, 3, this.spriteMedia + 5);
	}
	...
	if (this.uiTabSocialSubTab === 0) {
		for (let i = 0; i < this.friendListCount; i++) {
			...
			this.panelSocialList.addListEntry(
				...
				i,
				`${colour}${Utility.hashToUsername(this.friendListHashes[i])}` + `~${uiX + 126}~@whi@Remove`
			);
		}
		...
		for (let i = 0; i < this.ignoreListCount; i++) {
			this.panelSocialList.addListEntry(
				this.controlListSocialPlayers,
				i,
				`@yel@${Utility.hashToUsername(this.ignoreList[i])}` + `~${uiX + 126}~@whi@Remove`
			);
		}
	}
	...
	if (this.uiTabSocialSubTab === 0) {
		const friendIndex = this.panelSocialList.getListEntryIndex(this.controlListSocialPlayers);
		...
		if (friendIndex >= 0 && this.mouseX < uiX + 176) {
			...
			if (this.mouseX > uiX + 116) {
				...
				if (this.friendListOnline[friendIndex] < 200) {
					this.surface.drawStringCenter(
						`${username} is on world ` + (this.friendListOnline[friendIndex] - 9),
						uiX + HALF_WIDTH,
						uiY + 35,
						1,
						colours.white
					...
					this.surface.drawStringCenter(
						`${username} is on classic ` + (this.friendListOnline[friendIndex] - 219),
						uiX + HALF_WIDTH,
						uiY + 35,
						1,
						colours.white
		...
		const ignoreIndex = this.panelSocialList.getListEntryIndex(this.controlListSocialPlayers);

		if (ignoreIndex >= 0 && this.mouseX < uiX + 176 && this.mouseX > uiX + 116) {
			if (this.mouseX > uiX + 116) {
				this.surface.drawStringCenter(
					`${click} to remove ` + Utility.hashToUsername(this.ignoreList[ignoreIndex]),
					uiX + HALF_WIDTH,
					uiY + 35,
					1,
					colours.white
		...
		if (this.mouseButtonClick === 1 && this.uiTabSocialSubTab === 0) {
			...
			if (friendIndex >= 0 && mouseX < 176) {
				if (mouseX > 116) {
					...
					this.privateMessageTarget = this.friendListHashes[friendIndex];
