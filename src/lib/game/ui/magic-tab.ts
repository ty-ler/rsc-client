...
function drawUiTabMagic(noMenus) {
	...
	if (this.options.mobile) {
		...
		this.surface._drawSprite_from3(this.gameWidth - MENU_WIDTH - 3, 3, this.spriteMedia + 4);
	}
	...
	if (this.uiTabMagicSubTab === 0) {
		...
		for (let i = 0; i < GameData.spellCount; i++) {
			...
			this.panelMagic.addListEntry(
				...
				i,
				`${colourPrefix}Level ${GameData.spellLevel[i]}: ` + GameData.spellName[i]
			);
		}
		...
		if (spellIndex !== -1) {
			this.surface.drawString(
				`Level ${GameData.spellLevel[spellIndex]}` + `: ${GameData.spellName[spellIndex]}`,
				uiX + 2,
				uiY + 124,
				1,
				colours.yellow
		...
		for (let i = 0; i < GameData.prayerCount; i++) {
			...
			this.panelMagic.addListEntry(
				this.controlListMagic,
				i,
				`${colourPrefix}Level ${GameData.prayerLevel[i]}: ` + GameData.prayerName[i]
			);
		}
		...
		if (prayerIndex !== -1) {
			this.surface.drawStringCenter(
				`Level ${GameData.prayerLevel[prayerIndex]}: ` + GameData.prayerName[prayerIndex],
				uiX + HALF_WIDTH,
				uiY + 130,
				1,
				colours.yellow
		...
		if (this.mouseButtonClick === 1 && this.uiTabMagicSubTab === 0) {
			...
			if (spellIndex !== -1) {
				...
				if (GameData.spellLevel[spellIndex] > magicLevel) {
					...
					for (i = 0; i < GameData.spellRunesRequired[spellIndex]; i++) {
						const reagantId = GameData.spellRunesId[spellIndex][i];

						if (this.hasInventoryItems(reagantId, GameData.spellRunesCount[spellIndex][i])) {
							continue;
						}
