...
class GameConnection extends GameShell {
	...
	settingsBlockChat: number;
	settingsBlockPrivate: number;
	settingsBlockTrade: number;
	settingsBlockDuel: number;
	sessionID: Long;
	worldFullTimeout: number;
	moderatorLevel: number;
	autoLoginTimeout: number;
	packetLastRead: number;
	messageIndex: number;
	server: string;
	port: number;
	username: string;
	password: string;
	incomingPacket: Int8Array;
	friendListOnline: Int32Array;
	friendListHashes: any[];
	ignoreList: any[];
	static maxSocialListSize: number;
	messageTokens: Int32Array;
	packetStream: PacketStream;
	static maxReadTries: any;
	panelRecoverUser: any;
	controlRecoverQuestions: any;
	recentRecoverFail: any;
	loginScreen: number;
	controlRecoverAnswers: any;
	// options: any;
	...
	async register(username, password) {
		...
		try {
			...
			this.packetStream = new PacketStream(await this.createSocket(this.server, this.port));

			const encodedUsername = Utility.usernameToHash(username);

			this.packetStream.newPacket(clientOpcodes.SESSION);

			this.packetStream.putByte(encodedUsername.shiftRight(16).and(31).toInt());
	...
	showLoginScreenStatus(arg0: string, arg1: string) {
	...
	async login(username, password, reconnecting) {
		...
		try {
			...
			/**
			 * @TODO original line... what is going on here?
			 *  this.packetStream = new PacketStream(await this.createSocket(this.server, this.port), this);
			 */
			this.packetStream = new PacketStream(await this.createSocket(this.server, this.port));
	...
	controlRecoverInfo1(controlRecoverInfo1: any, arg1: string) {
		throw new Error('Method not implemented.');
	}
	controlRecoverInfo2(controlRecoverInfo2: any, arg1: string) {
		throw new Error('Method not implemented.');
	}
	controlRecoverOldPassword(controlRecoverOldPassword: any, arg1: string) {
		throw new Error('Method not implemented.');
	}
	controlRecoverNewPassword(controlRecoverNewPassword: any, arg1: string) {
		throw new Error('Method not implemented.');
	}
	controlRecoverConfirmPassword(controlRecoverConfirmPassword: any, arg1: string) {
		throw new Error('Method not implemented.');
	}
	...
	handleIncomingPacket(opcode: any, length: any, incomingPacket: any) {
		throw new Error('Method not implemented.');
	}
	...
	sortFriendsList() {
		...
		while (flag) {
			...
			for (let i = 0; i < this.friendListCount - 1; i++) {
				if (
					(this.friendListOnline[i] !== 255 && this.friendListOnline[i + 1] === 255) ||
					(this.friendListOnline[i] === 0 && this.friendListOnline[i + 1] !== 0)
				) {
	...
	friendRemove(encodedUsername) {
		...
		this.showServerMessage(
			`@pri@${Utility.hashToUsername(encodedUsername)} has been ` + 'removed from your friends list'
		);
	}
	showServerMessage(arg0: string) {
		throw new Error('Method not implemented.');
	}
