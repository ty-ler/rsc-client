...
class Surface {
	static anInt346: number;
	static anInt347: number;
	mudclient: any;
	image: any;
	anIntArray340: any;
	anIntArray341: any;
	...
	pixels: Int32Array;
	surfacePixels: any[];
	spriteColoursUsed: any[];
	spriteColourList: any[];
	spriteTranslate: boolean[];
	...
	imageData: any;
	static gameFonts: any;
	mudclientref: any;
	sinCosCache: any;
	static anInt348: number;
	static characterWidth: any;
	...
	constructor(width, height, limit, mudclient) {
		...
		this.spriteTranslate = Array(limit);
		...
		this.rgbPixels = new Uint8Array(this.pixels.buffer, this.pixels.byteOffset);
	...
	drawCircle(x, y, radius, colour, alpha) {
		...
		for (let yy = top; yy <= bottom; yy += vertInc) {
			...
			for (let i = j4; i <= k4; i++) {
				...
				const newColour =
					(((red + bgRed) >> 8) << 16) + (((green + bgGreen) >> 8) << 8) + ((blue + bgBlue) >> 8);
	...
	drawBoxAlpha(x, y, width, height, colour, alpha) {
		...
		for (let i = 0; i < height; i += vertInc) {
			for (let j = -width; j < 0; j++) {
				const backgroundRed = ((this.pixels[pixelIdx] >> 16) & 0xff) * backgroundAlpha;

				const backgroundGreen = ((this.pixels[pixelIdx] >> 8) & 0xff) * backgroundAlpha;

				const backgroundBlue = (this.pixels[pixelIdx] & 0xff) * backgroundAlpha;
		...
		for (let i = 0; i < height; i += vertInc) {
			if (i + y >= this.boundsTopY && i + y < this.boundsBottomY) {
				const newColour =
					...
					(((bottomGreen * i + topGreen * (height - i)) / height) << 8) +
	...
	parseSprite(spriteID, spriteData, indexData, frameCount) {
		...
		for (let id = spriteID; id < spriteID + frameCount; id++) {
			...
			this.spriteWidth[id] = Utility.getUnsignedShort(indexData, indexOffset);

			indexOffset += 2;

			this.spriteHeight[id] = Utility.getUnsignedShort(indexData, indexOffset);
			...
			if (unknown === 0) {
				for (let pixel = 0; pixel < size; pixel++) {
					this.spriteColoursUsed[id][pixel] = spriteData[spriteOffset++];
				...
				for (let x = 0; x < this.spriteWidth[id]; x++) {
					for (let y = 0; y < this.spriteHeight[id]; y++) {
						this.spriteColoursUsed[id][x + y * this.spriteWidth[id]] = spriteData[spriteOffset++];
	...
	readSleepWord(spriteID, spriteData) {
		const pixels = (this.surfacePixels[spriteID] = new Int32Array(SLEEP_WIDTH * SLEEP_HEIGHT));
	...
	drawActionBubble(x, y, scaleX, scaleY, sprite, alpha) {
		try {
			...
			if (y + scaleY >= this.boundsBottomY) scaleY -= y + scaleY - this.boundsBottomY + 1;
		...
		for (let k2 = -height; k2 < 0; k2 += yInc) {
			for (let l2 = -width; l2 < 0; l2++) {
				...
				if (i !== 0) {
					let i3 = dest[size];
					dest[size++] =
						((((i & 0xff00ff) * alpha + (i3 & 0xff00ff) * j2) & -16711936) +
							(((i & 0xff00) * alpha + (i3 & 0xff00) * j2) & 0xff0000)) >>
						8;
				} else {
					size++;
				}
		...
		for (let j2 = -height; j2 < 0; j2 += yInc) {
			for (let k2 = -width; k2 < 0; k2++) {
				...
				if (l2 !== 0) {
					...
					let i3 = dest[size];
					dest[size++] =
						((((l2 & 0xff00ff) * alpha + (i3 & 0xff00ff) * i2) & -16711936) +
							(((l2 & 0xff00) * alpha + (i3 & 0xff00) * i2) & 0xff0000)) >>
						8;
				} else {
					size++;
				}
			...
			for (let k3 = -k1; k3 < 0; k3 += yInc) {
				...
				for (let i4 = -j1; i4 < 0; i4++) {
					...
					if (i !== 0) {
						...
						dest[destPos++] =
							((((i & 0xff00ff) * alpha + (j4 & 0xff00ff) * i3) & -0xff0100) +
								(((i & 0xff00) * alpha + (j4 & 0xff00) * i3) & 0xff0000)) >>
							8;
					} else {
						destPos++;
					}
	...
	drawMinimapSprite(x, y, sprite, rotation, scale) {
		...
		if (!this.sinCosCache) {
			...
			for (let i = 0; i < 256; i++) {
				...
				this.sinCosCache[i + 256] = (Math.cos(i * 0.02454369) * 32768) | 0;
			}
		}
		...
		for (let i = k6; i < l6; i++) {
			...
			if (k11 - j11 <= 0) {
				...
				let i12 = (((this.anIntArray343[i] << 9) - l11) / (k11 - j11)) | 0;

				let j12 = this.anIntArray344[i] << 9;

				let k12 = (((this.anIntArray345[i] << 9) - j12) / (k11 - j11)) | 0;
				...
				if (j3 !== 0) {
					for (let k7 = k6; k7 < k6 + l6; k7++) {
						...
						if (i !== 0) {
							...
							if (k3 === l3 && l3 === i4) {
								dest[k7 + destPos] =
									(((k3 * j4) >> 8) << 16) + (((l3 * k4) >> 8) << 8) + ((i4 * l4) >> 8);
							} else if (k3 === 255 && l3 === i4) {
								dest[k7 + destPos] =
									(((k3 * i5) >> 8) << 16) + (((l3 * j5) >> 8) << 8) + ((i4 * k5) >> 8);
							} else {
								dest[k7 + destPos] = i;
							}
						}
			...
			for (let i2 = -l; i2 < 0; i2++) {
				...
				if (j2 > 30) {
					if (j2 >= 230) {
						...
						let k2 = ai[k];
						ai[k++] =
							((((i & 0xff00ff) * j2 + (k2 & 0xff00ff) * (256 - j2)) & 0xff00ff00) +
								(((i & 0xff00) * j2 + (k2 & 0xff00) * (256 - j2)) & 0xff0000)) >>
							8;
					}
				} else {
					k++;
	...
	drawTabs(x, y, width, height, tabs, selected) {
		...
		for (let i = 0; i < tabs.length; i += 1) {
			...
			this.drawStringCenter(tabs[i], x + offsetX + ((tabWidth / 2) | 0), y + 16, 4, BLACK);
