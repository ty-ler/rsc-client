...
function drawDialogShop() {
			...
			if (this.shopSelectedItemIndex >= 0) {
				...
				if (itemID !== -1) {
					if (
						...
						const itemPrice = ((priceMod * GameData.itemBasePrice[itemID]) / 100) | 0;
		...
		for (let col = 0; col < 8; col++) {
			...
			if (this.shopItem[itemIndex] !== -1) {
				this.surface._spriteClipping_from9(
					...
					48,
					32,
					this.spriteItem + GameData.itemPicture[this.shopItem[itemIndex]],
	...
	if (selectedItemID !== -1) {
		if (this.shopItemCount[this.shopSelectedItemIndex] > 0) {
			...
			this.surface.drawString(
				`Buy a new ${GameData.itemName[selectedItemID]} for ` + `${itemPrice}gp`,
				dialogX + 2,
				dialogY + 214,
				1,
				colours.yellow
		...
		if (this.getInventoryCount(selectedItemID) > 0) {
			...
			this.surface.drawStringRight(
				`Sell your ${GameData.itemName[selectedItemID]} for ` + `${itemPrice}gp`,
				dialogX + 405,
				dialogY + 239,
				1,
				colours.yellow
