...
function drawDialogTradeConfirm() {
	...
	this.surface.drawStringCenter(
		'Please confirm your trade with @yel@' + Utility.hashToUsername(this.tradeRecipientConfirmHash),
		DIALOG_X + 234,
		DIALOG_Y + 12,
		1,
		colours.white
	...
	for (let i = 0; i < this.tradeConfirmItemsCount; i++) {
		...
		if (GameData.itemStackable[this.tradeConfirmItems[i]] === 0) {
			itemLine += ' x ' + Utility.formatConfirmAmount(this.tradeConfirmItemCount[i]);
		}
	...
	for (let i = 0; i < this.tradeRecipientConfirmItemsCount; i++) {
		...
		if (GameData.itemStackable[this.tradeRecipientConfirmItems[i]] === 0) {
			itemLine += ' x ' + Utility.formatConfirmAmount(this.tradeRecipientConfirmItemCount[i]);
		}
