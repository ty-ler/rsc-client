...
class World {
	...
	landscapePack: any;
	mapPack: any;
	memberLandscapePack: any;
	memberMapPack: any;
	worldInitialised: boolean;
	objectAdjacency: any;
	tileDirection: any;
	wallModels: any[];
	roofModels: any[];
	terrainColours: Int32Array;
	wallsNorthSouth: any;
	wallsRoof: any;
	terrainHeight: any;
	terrainColour: any;
	...
	aBoolean592: boolean;
	playerAlive: boolean;
	terrainHeightLocal: any;
	terrainModels: any[];
	localX: Int32Array;
	baseMediaSprite: number;
	scene: any;
	surface: any;
	static colourTransparent: any;
	...
	constructor(scene, surface) {
		...
		this.objectAdjacency = ndarray(new Int32Array(this.regionWidth * this.regionHeight), [
			this.regionWidth,
			this.regionHeight
		]);
		...
		this.routeVia = ndarray(new Int32Array(this.regionWidth * this.regionHeight), [
			this.regionWidth,
			this.regionHeight
		]);
		...
		this.terrainHeightLocal = ndarray(new Int32Array(this.regionWidth * this.regionHeight), [
			this.regionWidth,
			this.regionHeight
		]);
	...
	_loadSection_from4I(x, y, plane, chunk) {
		const mapName = 'm' + plane + ((x / 10) | 0) + (x % 10) + ((y / 10) | 0) + (y % 10);
	...
	loadSection(...args) {
		switch (args.length) {
			case 3:
				return this._loadSection_from3(args[0], args[1], args[2]);
			case 4:
				if (typeof args[3] === 'number') {
					return this._loadSection_from4I(args[0], args[1], args[2], args[3]);
				}

				return this._loadSection_from4(args[0], args[1], args[2], args[3]);
		}
	}
	...
	getTileDecoration(...args) {
		switch (args.length) {
			case 3:
				return this._getTileDecoration_from3(args[0], args[1]);
			case 4:
				return this._getTileDecoration_from4(args[0], args[1], args[2], args[3]);
		}
	}
	...
	setObjectAdjacency(...args) {
		switch (args.length) {
			case 4:
				return this._setObjectAdjacency_from4(args[0], args[1], args[2], args[3]);
			case 3:
				return this._setObjectAdjacency_from3(args[0], args[1], args[2]);
		}
	}
			...
			for (let j2 = 0; j2 < this.regionWidth; j2++) {
				for (let i3 = 0; i3 < this.regionHeight; i3++) {
					...
					if (
						...
						GameData.tileType[this.getTileDecoration(j2, i3, plane) - 1] === 4
					) {
						i4 = 0;
					}
					...
					if (
						...
						GameData.tileType[this.getTileDecoration(j2 - 1, i3, plane) - 1] === 4
					) {
						i4 = 0;
					}
					...
					if (
						...
						GameData.tileType[this.getTileDecoration(j2, i3 - 1, plane) - 1] === 4
					) {
						i4 = 0;
					}
					...
					if (
						...
						GameData.tileType[this.getTileDecoration(j2 - 1, i3 - 1, plane) - 1] === 4
					) {
						i4 = 0;
					}
					...
					if (this.getTileDecoration(lx, ly, plane) > 0) {
						...
						if (decorationTileType === 5) {
							if (this.getWallDiagonal(lx, ly) > 0 && this.getWallDiagonal(lx, ly) < 24000) {
								if (
									this.getTileDecoration(lx - 1, ly, plane, colour_2) !== World.colourTransparent &&
									this.getTileDecoration(lx, ly - 1, plane, colour_2) !== World.colourTransparent
								) {
							...
							(this.getWallDiagonal(lx, ly) > 0 && this.getWallDiagonal(lx, ly) < 24000)
						) {
							if (
								this.getTileType(lx - 1, ly, plane) !== tileType &&
			...
			for (let k4 = 1; k4 < 95; k4++) {
				for (let i6 = 1; i6 < 95; i6++) {
					if (
						...
						GameData.tileType[this.getTileDecoration(k4, i6, plane) - 1] === 4
					) {
						...
						this.getTileDecoration(k4, i6, plane) === 0 ||
						GameData.tileType[this.getTileDecoration(k4, i6, plane) - 1] !== 3
					) {
						if (
							this.getTileDecoration(k4, i6 + 1, plane) > 0 &&
							GameData.tileType[this.getTileDecoration(k4, i6 + 1, plane) - 1] === 4
						) {
						...
						if (
							...
							GameData.tileType[this.getTileDecoration(k4 + 1, i6, plane) - 1] === 4
						) {
		...
		for (let i2 = 0; i2 < 95; i2++) {
			for (let k2 = 0; k2 < 95; k2++) {
				...
				if (
					k3 > 0 &&
					k3 < 12000 &&
					(GameData.wallObjectInvisible[k3 - 1] === 0 || this.aBoolean592)
				) {
				...
				if (
					k3 > 12000 &&
					k3 < 24000 &&
					(GameData.wallObjectInvisible[k3 - 12001] === 0 || this.aBoolean592)
				) {
