...
class WordFilter {
	static tldList: any[];
	static tldType: Int32Array;
	static badList: any[];
	static badCharIds: any[];
	static hostList: any[];
	static hostCharIds: any[];
	static hashFragments: Int32Array;
	static ignoreList: any;
	static forceLowerCase: any;
	static DEBUGTLD: any;
	static DEBUGWORD: any;
	...
	static readBuffer(buffer, wordList, charIDs) {
		for (let i = 0; i < wordList.length; i++) {
			...
			for (let j = 0; j < ids.length; j++) {
				ids[j] = [buffer.getUnsignedByte() & 0xff, buffer.getUnsignedByte() & 0xff];
			}
	...
	static filter(input) {
		...
		for (let i = 0; i < WordFilter.ignoreList.length; i++) {
			for (let j = -1; (j = input.indexOf(WordFilter.ignoreList[i], j + 1)) !== -1; ) {
	...
	static applyTLDFilter(input, input1, input2, tld, type) {
		...
		for (let i = 0; i <= input.length - tld.length; i++) {
			...
			if (l >= tld.length) {
				...
				const startMatch = WordFilter.getAsteriskCount(input, input1, i);

				const endMatch = WordFilter.getAsteriskCount2(input, input2, inputCharCount - 1);
	...
	static applyWordFilter(input, wordList, charIDs) {
		...
		for (let charIndex = 0; charIndex <= input.length - wordList.length; charIndex++) {
			...
			while (inputCharCount < input.length) {
				...
				if (
					k < wordList.length &&
					(l = WordFilter.compareLettersSymbols(wordList[k], inputChar, nextChar)) > 0
				) {
					inputCharCount += l;
					k++;
					continue;
			...
			if (k >= wordList.length) {
				...
				if (!specialChar) {
					...
					if (charIDs && WordFilter.compareCharIds(charIDs, previousID, currentID)) {
						filter = false;
					}
				} else {
					let flag2 = false;
					...
					if (
						charIndex - 1 < 0 ||
						(WordFilter.isSpecial(input[charIndex - 1]) && input[charIndex - 1] !== C_SINGLE_QUOTE)
					) {
						flag2 = true;
					}
						...
						for (; !flag4 && j1 < inputCharCount; j1++) {
							if (j1 >= 0 && (!WordFilter.isSpecial(input[j1]) || input[j1] === C_SINGLE_QUOTE)) {
								...
								for (k1 = 0; k1 < 3; k1++) {
									if (
										j1 + k1 >= input.length ||
										(WordFilter.isSpecial(input[j1 + k1]) && input[j1 + k1] !== C_SINGLE_QUOTE)
									) {
										break;
									}
								...
								if (
									k1 < 3 &&
									j1 - 1 >= 0 &&
									(!WordFilter.isSpecial(input[j1 - 1]) || input[j1 - 1] === C_SINGLE_QUOTE)
								) {
									flag5 = false;
								}
	...
	static compareCharIds(charIdData, prevCharId, curCharId) {
		...
		while (first !== last && first + 1 !== last) {
			...
			if (
				...
				(prevCharId === charIdData[middle][0] && curCharId < charIdData[middle][1])
			) {
				last = middle;
			} else {
				first = middle;
