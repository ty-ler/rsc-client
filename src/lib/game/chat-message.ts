	...
	static descramble(buffer, offset, length) {
		try {
			...
			for (let i = 0; i < length; i++) {
				...
				if (leftShift === -1) {
					if (charMapIndex < 13) {
						ChatMessage.chars[newLength++] = ChatMessage.charMap[charMapIndex];
					} else {
						leftShift = charMapIndex;
					}
				} else {
					ChatMessage.chars[newLength++] =
						ChatMessage.charMap[(leftShift << 4) + charMapIndex - 195];
		...
		for (let i = 0; i < message.length; i++) {
			...
			if (leftShift === -1) {
				if (charMapIndex < 13) {
					...
					ChatMessage.scrambledBytes[offset++] = charMapIndex & 0xff;
				}
			} else if (charMapIndex < 13) {
				ChatMessage.scrambledBytes[offset++] = ((leftShift << 4) + charMapIndex) & 0xff;

				leftShift = -1;
			} else {
				ChatMessage.scrambledBytes[offset++] = ((leftShift << 4) + (charMapIndex >> 4)) & 0xff;
