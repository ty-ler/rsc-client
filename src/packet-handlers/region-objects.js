	...
	[serverOpcodes.REGION_OBJECTS]: function (data, size) {
		for (let offset = 1; offset < size; ) {
			if (Utility.getUnsignedByte(data[offset]) === 255) {
				...
				for (let i = 0; i < this.objectCount; i++) {
					if (this.objectX[i] !== areaX || this.objectY[i] !== areaY) {
						if (i !== objectIndex) {
				...
				if (objectID !== 60000) {
					...
					const modelX = (((areaX + areaX + width) * this.magicLoc) / 2) | 0;

					const modelY = (((areaY + areaY + height) * this.magicLoc) / 2) | 0;
					...
					model.translate(modelX, -this.world.getElevation(modelX, modelY), modelY);
