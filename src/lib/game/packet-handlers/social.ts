...
export default {
	[serverOpcodes.FRIEND_LIST]: function (data) {
		...
		for (let i = 0; i < this.friendListCount; i++) {
			...
			this.friendListOnline[i] = Utility.getUnsignedByte(data[10 + i * 9]);
		}
			...
			if (this.friendListHashes[i].equals(encodedUsername)) {
				if (this.friendListOnline[i] === 0 && world !== 0) {
					this.showServerMessage(
						`@pri@${Utility.hashToUsername(encodedUsername)} ` + 'has logged in'
					);
				}

				if (this.friendListOnline[i] !== 0 && world === 0) {
					this.showServerMessage(
						`@pri@${Utility.hashToUsername(encodedUsername)} ` + 'has logged out'
					);
				}
	...
	[serverOpcodes.FRIEND_MESSAGE]: function (data, size) {
		...
		this.messageIndex = (this.messageIndex + 1) % GameConnection.maxSocialListSize;
		...
		this.showServerMessage(`@pri@${Utility.hashToUsername(from)}: tells you ${message}`);
	},
