...
function drawUiTabMinimap(noMenus) {
	...
	if (this.options.mobile) {
		...
		this.surface._drawSprite_from3(this.gameWidth - MENU_WIDTH - 3, 3, this.spriteMedia + 2);
	}
	...
	for (let i = 0; i < this.objectCount; i++) {
		let objectX =
			(((this.objectX[i] * this.magicLoc + 64 - this.localPlayer.currentX) * 3 * scale) / 2048) | 0;

		let objectY =
			(((this.objectY[i] * this.magicLoc + 64 - this.localPlayer.currentY) * 3 * scale) / 2048) | 0;
	...
	for (let i = 0; i < this.groundItemCount; i++) {
		let itemX =
			(((this.groundItemX[i] * this.magicLoc + 64 - this.localPlayer.currentX) * 3 * scale) /
				2048) |
			0;

		let itemY =
			(((this.groundItemY[i] * this.magicLoc + 64 - this.localPlayer.currentY) * 3 * scale) /
				2048) |
			0;
	...
	for (let i = 0; i < this.npcCount; i++) {
		...
		let npcX = (((npc.currentX - this.localPlayer.currentX) * 3 * scale) / 2048) | 0;

		let npcY = (((npc.currentY - this.localPlayer.currentY) * 3 * scale) / 2048) | 0;
	...
	for (let i = 0; i < this.playerCount; i++) {
		...
		let otherPlayerX = (((player.currentX - this.localPlayer.currentX) * 3 * scale) / 2048) | 0;

		let otherPlayerY = (((player.currentY - this.localPlayer.currentY) * 3 * scale) / 2048) | 0;
		...
		for (let j = 0; j < this.friendListCount; j++) {
			if (!player.hash.equals(this.friendListHashes[j]) || this.friendListOnline[j] !== 255) {
				continue;
			}
	...
	if (mouseX >= 0 && mouseY >= 0 && mouseX < WIDTH + 40 && mouseY < HEIGHT) {
		let deltaY = (((this.mouseX - (uiX + HALF_WIDTH)) * 16384) / (3 * scale)) | 0;

		let deltaX = (((this.mouseY - (uiY + HALF_HEIGHT)) * 16384) / (3 * scale)) | 0;
