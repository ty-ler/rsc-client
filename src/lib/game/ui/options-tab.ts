...
function drawUiTabOptions(noMenus) {
	...
	if (this.options.mobile) {
		...
		this.surface._drawSprite_from3(this.gameWidth - MENU_WIDTH - 3, 3, this.spriteMedia + 6);
	}
	...
	this.surface.drawString(
		'Camera angle mode - ' + (this.optionCameraModeAuto ? '@gre@Auto' : '@red@Manual'),
		x,
		y,
		1,
		colours.white
	...
	this.surface.drawString(
		this.options.mobile
			? 'Single tap mode - ' + (this.optionMouseButtonOne ? '@gre@on' : '@red@off')
			: 'Mouse buttons - ' + (this.optionMouseButtonOne ? '@red@One' : '@gre@Two'),
		x,
		y,
		1,
		colours.white
	...
	if (this.members) {
		this.surface.drawString(
			'Sound effects - ' + (this.optionSoundDisabled ? '@red@off' : '@gre@on'),
			x,
			y,
			1,
			colours.white
	...
	this.surface.drawString(
		'Block chat messages: ' + (!this.settingsBlockChat ? '@red@<off>' : '@gre@<on>'),
		uiX + 3,
		y,
		1,
		colours.white
	...
	this.surface.drawString(
		'Block private messages: ' + (!this.settingsBlockPrivate ? '@red@<off>' : '@gre@<on>'),
		uiX + 3,
		y,
		1,
		colours.white
	...
	this.surface.drawString(
		'Block trade requests: ' + (!this.settingsBlockTrade ? '@red@<off>' : '@gre@<on>'),
		uiX + 3,
		y,
		1,
		colours.white
	...
	if (this.members) {
		this.surface.drawString(
			'Block duel requests: ' + (!this.settingsBlockDuel ? '@red@<off>' : '@gre@<on>'),
			uiX + 3,
			y,
			1,
			colours.white
...
export default { drawUiTabOptions };
