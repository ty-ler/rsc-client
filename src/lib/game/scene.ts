...
class Scene {
	static frustumNearZ: number;
	static frustumFarZ: number;
	static frustumMinX: number;
	static frustumMaxX: number;
	static frustumMinY: number;
	static frustumMaxY: number;
	lastVisiblePolygonsCount: number;
	anIntArray377: any;
	textureCount: number;
	textureColoursUsed: any;
	textureColourList: any;
	textureDimension: any;
	textureLoadedNumber: any;
	texturePixels: any;
	textureBackTransparent: any;
	textureColours64: any;
	textureColours128: any;
	scanlines: any;
	minY: number;
	maxY: number;
	interlace: boolean;
	...
	fogZDistance: number;
	wideBand: boolean;
	mousePickingActive: boolean;
	mousePickedMax: number;
	mousePickedModels: any[];
	mousePickedFaces: Int32Array;
	width: number;
	clipX: number;
	clipY: number;
	baseX: number;
	...
	static sin512Cache: any;
	static sinCosCache: any;
	view: any;
	static textureCountLoaded: any;
	...
	constructor(surface, maxModelCount, polygonCount, spriteCount) {
		...
		for (let i = 0; i < 1024; i++) {
			...
			Scene.sinCosCache[i + 1024] = (Math.cos(i * 0.00613592315) * 32768) | 0;
		}
	}
				...
				for (let k11 = 0; k11 < l10; k11++) {
					...
					if (gameModel_2.projectVertexZ[k2] >= this.clipNear) {
						...
						if (gameModel_2.projectVertexZ[k2] > this.fogZDistance) {
							this.vertexShade[k8] +=
								((gameModel_2.projectVertexZ[k2] - this.fogZDistance) / this.fogZFalloff) | 0;
						}
						...
						if (gameModel_2.projectVertexZ[k9] >= this.clipNear) {
							...
							let i5 =
								...
								((((gameModel_2.projectVertexX[k2] - gameModel_2.projectVertexX[k9]) *
									(gameModel_2.projectVertexZ[k2] - this.clipNear)) /
									k7) |
									0);

							let j6 =
								gameModel_2.projectVertexY[k2] -
								((((gameModel_2.projectVertexY[k2] - gameModel_2.projectVertexY[k9]) *
									(gameModel_2.projectVertexZ[k2] - this.clipNear)) /
									k7) |
									0);
	...
	initialisePolygon3D(i) {
		...
		if (faceCameraNormalScale === -1) {
			...
			for (
				;
				t1 > 25000 || t2 > 25000 || t3 > 25000 || t1 < -25000 || t2 < -25000 || t3 < -25000;
				t3 >>= 1
			) {
			...
			gameModel.normalMagnitude[face] =
				(this.normalMagnitude * Math.sqrt(t1 * t1 + t2 * t2 + t3 * t3)) | 0;
		} else {
	...
	setTexturePixels(id) {
		...
		for (let x = 0; x < textureWidth; x++) {
			for (let y = 0; y < textureWidth; y++) {
				let colour =
					this.textureColourList[id][this.textureColoursUsed[id][y + x * textureWidth] & 0xff];
				colour &= 0xf8f8ff;
