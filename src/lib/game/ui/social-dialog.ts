...
function drawDialogSocialInput() {
	if (this.mouseButtonClick !== 0) {
		...
		if (
			...
			(this.mouseX < 6 || this.mouseY < 145 || this.mouseX > 506 || this.mouseY > 215)
		) {
			this.showDialogSocialInput = 0;
			return;
		}
	...
	if (this.showDialogSocialInput === 1) {
		...
		if (this.inputTextFinal.length > 0) {
			...
			if (username.length > 0 && !encodedUsername.equals(this.localPlayer.hash)) {
		...
		if (this.inputPMFinal.length > 0) {
			...
			this.sendPrivateMessage(this.privateMessageTarget, ChatMessage.scrambledBytes, length);

			message = ChatMessage.descramble(ChatMessage.scrambledBytes, 0, length);
