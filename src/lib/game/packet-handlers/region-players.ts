...
export default {
	[serverOpcodes.REGION_PLAYERS]: function (data, size) {
		...
		const hasLoadedRegion = this.loadNextRegion(this.localRegionX, this.localRegionY);
		...
		this.localPlayer = this.createPlayer(this.localPlayerServerIndex, playerX, playerY, sprite);
