...
class GameModel {
	...
	vertexY: any;
	vertexZ: any;
	vertexTransformedX: any;
	vertexTransformedY: any;
	vertexTransformedZ: any;
	faceTransStateThing: any;
	faceBoundLeft: any;
	faceBoundRight: any;
	faceBoundBottom: any;
	faceBoundTop: any;
	...
	static fromBytes(data, offset) {
		...
		for (let i = 0; i < numFaces; i++) {
			gameModel.faceVertices[i] = new Int32Array(gameModel.faceNumVertices[i]);

			for (let j = 0; j < gameModel.faceNumVertices[i]; j++) {
				if (numVertices < 256) {
					gameModel.faceVertices[i][j] = data[offset++] & 0xff;
				} else {
					gameModel.faceVertices[i][j] = Utility.getUnsignedShort(data, offset);
					offset += 2;
				}
			}
		}
			...
			for (let srcF = 0; srcF < source.numFaces; srcF++) {
				...
				if (transState) {
					if (count > 1) {
						...
						for (let j = 0; j < source.faceTransStateThing[srcF].length; j++) {
							this.faceTransStateThing[dstF][j + 1] = source.faceTransStateThing[srcF][j];
						}
					} else {
						this.faceTransStateThing[dstF] = new Int32Array(
						...
						for (let j = 0; j < source.faceTransStateThing[srcF].length; j++) {
							this.faceTransStateThing[dstF][j] = source.faceTransStateThing[srcF][j];
						}
					}
				}
			}
	...
	applyRotation(yaw, roll, pitch) {
		for (let i = 0; i < this.numVertices; i++) {
			if (pitch !== 0) {
				...
				let x = (this.vertexTransformedY[i] * sin + this.vertexTransformedX[i] * cos) >> 15;

				this.vertexTransformedY[i] =
					(this.vertexTransformedY[i] * cos - this.vertexTransformedX[i] * sin) >> 15;
			...
			if (yaw !== 0) {
				...
				let y = (this.vertexTransformedY[i] * cos - this.vertexTransformedZ[i] * sin) >> 15;

				this.vertexTransformedZ[i] =
					(this.vertexTransformedY[i] * sin + this.vertexTransformedZ[i] * cos) >> 15;
			...
			if (roll !== 0) {
				...
				let x = (this.vertexTransformedZ[i] * sin + this.vertexTransformedX[i] * cos) >> 15;

				this.vertexTransformedZ[i] =
					(this.vertexTransformedZ[i] * cos - this.vertexTransformedX[i] * sin) >> 15;
				this.vertexTransformedX[i] = x;
			}
		}
	}
	...
	applyShear(xy, xz, yx, yz, zx, zy) {
		for (let i = 0; i < this.numVertices; i++) {
			if (xy !== 0) {
				this.vertexTransformedX[i] += (this.vertexTransformedY[i] * xy) >> 8;
			}

			if (xz !== 0) {
				this.vertexTransformedZ[i] += (this.vertexTransformedY[i] * xz) >> 8;
			}

			if (yx !== 0) {
				this.vertexTransformedX[i] += (this.vertexTransformedZ[i] * yx) >> 8;
			}

			if (yz !== 0) {
				this.vertexTransformedY[i] += (this.vertexTransformedZ[i] * yz) >> 8;
			}

			if (zx !== 0) {
				this.vertexTransformedZ[i] += (this.vertexTransformedX[i] * zx) >> 8;
			}

			if (zy !== 0) {
				this.vertexTransformedY[i] += (this.vertexTransformedX[i] * zy) >> 8;
			}
		}
	}
	...
	copy(...args) {
		...
		let gameModel = GameModel._from6([this], 1, autocommit, isolated, unlit, pickable);
	...
	readBase64(buffer) {
		for (; buffer[this.dataPtr] === 10 || buffer[this.dataPtr] === 13; this.dataPtr++);
