...
function drawUiTabInventory(noMenus) {
	...
	if (this.options.mobile) {
		...
		this.surface._drawSprite_from3(this.gameWidth - MENU_WIDTH - 3, 3, this.spriteMedia + 1);
	}
	...
	if (
		...
		const itemIndex = ((mouseX / SLOT_WIDTH) | 0) + ((mouseY / SLOT_HEIGHT) | 0) * 5;
		...
		if (itemIndex < this.inventoryItemsCount) {
			...
			if (this.selectedSpell >= 0) {
				...
				if (GameData.itemCommand[itemID] !== '') {
					this.menuItemText1[this.menuItemsCount] = GameData.itemCommand[itemID];
...
export default { drawUiTabInventory };
