...
class Utility {
	...
	static getUnsignedLong(buffer, offset) {
		return Long.fromInt(Utility.getUnsignedInt(buffer, offset) & 0xffffffff)
			.shiftLeft(32)
			.add(new Long(Utility.getUnsignedInt(buffer, offset + 4) & 0xffffffff));
	}
	...
	static getBitMask(buffer, start, length) {
		...
		for (; length > bitOffset; bitOffset = 8) {
			bits += (buffer[byteOffset++] & Utility.bitmask[bitOffset]) << (length - bitOffset);
		...
		if (length === bitOffset) {
			...
			bits += (buffer[byteOffset] >> (bitOffset - length)) & Utility.bitmask[length];
		}
	...
	static unpackData(fileName, extraSize, archiveData, fileData) {
		const numEntries = ((archiveData[0] & 0xff) * 256 + (archiveData[1] & 0xff)) | 0;
	...
	static formatConfirmAmount(amount) {
		...
		if (formatted.length > 8) {
			formatted =
				`@gre@${formatted.substring(0, formatted.length - 8)}` + ` million @whi@(${formatted})`;
		} else if (formatted.length > 4) {
			formatted = `@cya@${formatted.substring(0, formatted.length - 4)}` + `K @whi@(${formatted})`;
		}
...
Utility.bitmask = new Int32Array([
	0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383, 32767, 65535, 0x1ffff,
