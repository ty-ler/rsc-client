...
function drawChatMessageTabs() {
	...
	if (this.options.mobile) {
		...
		this.surface.drawMinimapSprite(x + HBAR_WIDTH / 2 - 103, y, this.spriteMedia + 23, 128, 128);
...
async function handleMesssageTabsInput() {
	...
	if (this.options.mobile && this.lastMouseButtonDown) {
		if (!this.panelMessageTabs.controlText[this.controlTextListAll].length) {
