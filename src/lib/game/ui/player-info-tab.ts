...
const EQUIPMENT_STAT_NAMES = ['Armour', 'WeaponAim', 'WeaponPower', 'Magic', 'Prayer'];
...
function drawUiTabPlayerInfo(noMenus) {
	...
	if (this.options.mobile) {
		...
		this.surface._drawSprite_from3(this.gameWidth - MENU_WIDTH - 3, 3, this.spriteMedia + 3);
	}
	...
	if (this.uiTabPlayerInfoSubTab === 0) {
		...
		for (let i = 0; i < 3; i++) {
			...
			if (i < 2) {
				this.surface.drawString(
					`${EQUIPMENT_STAT_NAMES[i + 3]}:@yel@` + this.playerStatEquipment[i + 3],
					uiX + HALF_WIDTH + 25,
					y,
					1,
					colours.white
		...
		if (selectedSkill !== -1) {
			...
			for (let i = 0; i < 98; i++) {
				if (this.playerExperience[selectedSkill] >= EXPERIENCE_ARRAY[i]) {
