...
class Panel {
	static baseSpriteStart: number;
	controlCount: number;
	mouseX: number;
	mouseY: number;
	mouseLastButtonDown: number;
	mouseButtonDown: number;
	mouseMetaButtonHeld: number;
	mouseScrollDelta: number;
	focusControlIndex: number;
	aBoolean219: boolean;
	surface: any;
	maxControls: any;
	controlShown: boolean[];
	controlListScrollbarHandleDragged: boolean[];
	...
	controlText: any[];
	controlListEntries: any[];
	colourScrollbarTop: any;
	colourScrollbarBottom: any;
	colourScrollbarHandleLeft: any;
	colourScrollbarHandleMid: any;
	colourScrollbarHandleRight: any;
	colourRoundedBoxOut: any;
	colourRoundedBoxMid: any;
	colourRoundedBoxIn: any;
	colourBoxTopNBottom: any;
	colourBoxTopNBottom2: any;
	colourBoxLeftNRight2: any;
	colourBoxLeftNRight: any;
	static redMod: any;
	static greenMod: any;
	static blueMod: any;
	static drawBackgroundArrow: any;
	static textListEntryHeightMod: any;
	...
	constructor(surface, max) {
		...
		this.controlShown = Array(max);
		this.controlListScrollbarHandleDragged = Array(max);
		this.controlMaskText = new Int8Array(max);
		this.controlClicked = Array(max);
		...
		if (lastButton === 1) {
			for (let i = 0; i < this.controlCount; i++) {
				...
				if (
					...
					this.controlListEntryMouseButtonDown[i] = 1 - this.controlListEntryMouseButtonDown[i];
				}
			}
		}
	...
	keyPress(key) {
		...
		if (
			...
			if (key === 8 && inputLen > 0) {
				this.controlText[this.focusControlIndex] = this.controlText[this.focusControlIndex].slice(
					0,
					inputLen - 1
				);
			}
			...
			if (this.controlShown[i]) {
				if (this.controlType[i] === controlTypes.TEXT) {
					...
					this.drawText(
						i,
						this.controlX[i] -
							((this.surface.textWidth(this.controlText[i], this.controlTextSize[i]) / 2) | 0),
						this.controlY[i],
						this.controlText[i],
						this.controlTextSize[i]
					);
	...
	drawTextInput(control, x, y, width, height, text, textSize) {
		...
		if (this.focusControlIndex === control) {
			...
			if (mudclient.options.mobile && caret !== -1) {
				displayText = displayText.slice(0, caret) + '*' + displayText.slice(caret);
			} else {
				displayText += '*';
			}
		}
	...
	drawTextList(
		...
		const displayedEntryCount = (height / this.surface.textHeight(textSize)) | 0;
		...
		if (displayedEntryCount < listEntryCount) {
			...
			if (
				this.mouseButtonDown === 1 &&
				((this.mouseX >= cornerTopRight && this.mouseX <= cornerTopRight + 12) ||
					(this.mouseX >= cornerTopRight - 12 &&
				...
				if (this.mouseY > y + 12 && this.mouseY < y + height - 12) {
					...
					const l3 = this.mouseY - y - 12 - ((cornerBottomLeft / 2) | 0);
		...
		const listStartY = height - displayedEntryCount * this.surface.textHeight(textSize);

		let listY = (y + ((this.surface.textHeight(textSize) * 5) / 6 + listStartY / 2)) | 0;

		for (let entry = listEntryPosition; entry < listEntryCount; entry++) {
			this.drawString(control, x + 2, listY, listEntries[entry], textSize);

			listY += this.surface.textHeight(textSize) - Panel.textListEntryHeightMod;
	...
	drawOptionListHoriz(control, x, y, textSize, listEntries) {
		...
		for (let idx = 0; idx < listEntryCount; idx++) {
			...
			if (
				this.mouseX >= left &&
				this.mouseX <= left + this.surface.textWidth(listEntries[idx], textSize) &&
				this.mouseY <= bottom &&
			...
			this.surface.drawString(listEntries[idx], left, bottom, textSize, textColour);
	...
	drawOptionListVert(control, x, y, textSize, listEntries) {
		...
		let listTotalTextHeightMid =
			y - (((this.surface.textHeight(textSize) * (listEntryCount - 1)) / 2) | 0);
		...
		for (let idx = 0; idx < listEntryCount; idx++) {
			...
			const entryTextWidth = this.surface.textWidth(listEntries[idx], textSize);

			if (
				this.mouseX >= x - ((entryTextWidth / 2) | 0) &&
				this.mouseX <= x + ((entryTextWidth / 2) | 0) &&
				this.mouseY - 2 <= listTotalTextHeightMid &&
				this.mouseY - 2 > listTotalTextHeightMid - this.surface.textHeight(textSize)
			) {
				if (this.controlUseAlternativeColour[control]) {
					colour = 0x808080;
				} else {
	...
	drawTextListInteractive(
		...
		let listY = y + (((((this.surface.textHeight(textSize) * 5) / 6) | 0) + listStartY / 2) | 0);
		...
		for (let k3 = listEntryPosition; k3 < listEntryCount; k3++) {
			...
			this.surface.drawString(listEntries[k3], x + 2, listY, textSize, textColour);
	...
	setMobileFocus(control, text) {
		...
		const isListInput = this.controlType[control] === controlTypes.LIST_INPUT;
