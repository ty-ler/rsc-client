...
class GameShell {
	...
	_graphics: Graphics;
	options: {
		middleClickCamera: boolean;
		mouseWheel: boolean;
		resetCompass: boolean;
		zoomCamera: boolean;
		showRoofs: boolean;
		remainingExperience: boolean;
		totalExperience: boolean;
		wordFilter: boolean;
		accountManagement: boolean;
		fpsCounter: boolean;
		retryLoginOnDisconnect: boolean;
		mobile: boolean;
	};
	middleButtonDown: boolean;
	mouseScrollDelta: number;
	mouseActionTimeout: number;
	logoHeaderText: any;
	mouseX: number;
	mouseY: number;
	mouseButtonDown: number;
	lastMouseButtonDown: number;
	timings: any[];
	stopTimeout: number;
	interlaceTimer: number;
	...
	maxDrawTime: number;
	loadingStep: number;
	hasRefererLogoNotUsed: boolean;
	loadingProgessText: string;
	fontTimesRoman15: Font;
	fontHelvetica13b: Font;
	fontHelvetica12: Font;
	keyLeft: boolean;
	keyRight: boolean;
	keyUp: boolean;
	keyDown: boolean;
	...
	inputPMFinal: string;
	disableEndClick: boolean;
	touchMoving: any;
	toggleKeyboard: boolean;
	mobileInput: HTMLInputElement;
	mobilePassword: HTMLInputElement;
	mobileInputEl: any;
	mobileInputCaret: number;
	lastMobileInput: any;
	originRotation: any;
	cameraRotation: any;
	originMouseX: any;
	fps: number;
	rightClickTimeout: ReturnType<typeof setTimeout>;
	keyboardUpdateInterval: ReturnType<typeof setTimeout>;
	...
	async startApplication(width, height, title) {
		...
		if (this.options.mobile) {
			this._canvas.addEventListener('touchstart', (e: any) => {
				//e.preventDefault();
			...
			this._canvas.addEventListener('touchend', (e: any) => {
				e.preventDefault();
		...
		if (this.options.mobile) {
			...
			this.mobileInput.addEventListener('keydown', this.mobileKeyDown.bind(this));

			this.mobilePassword.addEventListener('keydown', this.mobileKeyDown.bind(this));

			this.mobileInput.addEventListener('blur', this.closeKeyboard.bind(this));

			this.mobilePassword.addEventListener('blur', this.closeKeyboard.bind(this));
	...
	keyPressed(e) {
		...
		let charCode = e.key && e.key.length === 1 ? e.key.charCodeAt(0) : 65535;
	...
	handleMouseDown(mouseButtonDown: any, x: number, y: number) {
		throw new Error('Method not implemented.');
	}

	mouseWheel(e) {
		if (!this.options.mouseWheel || document.activeElement !== this._canvas) {
			return;
		}
	...
	async loadJagex() {
		...
		const fontsJag = await this.readDataFile(`fonts${version.FONTS}.jag`, 'Game fonts', 5);
	...
	drawString(graphics, string, font, x, y) {
		...
		graphics.drawString(string, x - ((width / 2) | 0), y + ((height / 4) | 0));
	}
	...
	async readDataFile(file, description, percent) {
		...
		const archiveSize = ((header[0] & 0xff) << 16) + ((header[1] & 0xff) << 8) + (header[2] & 0xff);

		const archiveSizeCompressed =
			((header[3] & 0xff) << 16) + ((header[4] & 0xff) << 8) + (header[5] & 0xff);
		...
		while (read < archiveSizeCompressed) {
			...
			this.showLoadingProgress(
				percent,
				`Loading ${description} - ` + ((5 + (read * 95) / archiveSizeCompressed) | 0) + '%'
			);
		}
